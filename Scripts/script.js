...
async function showResponse() {
    ...
    function displaySome(total) {
        ...
        firstField.innerHTML = "<h4>First Name(s):</h4>"
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (answer.entry[i].resource.name[0].given[1] != undefined) {
                firstDiv.innerHTML = `<p>${answer.entry[i].resource.name[0].given[0]} ${answer.entry[i].resource.name[0].given[1]}</p>`;
            } else {
                firstDiv.innerHTML = `<p>${answer.entry[i].resource.name[0].given[0]}</p>`;
            } //if else checks to see if middle name present - if yes display, if no then don't (otherwise shows as undefined)
